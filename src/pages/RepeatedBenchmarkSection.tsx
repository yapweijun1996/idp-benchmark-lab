  ...
  return (
    <div className="progress-panel" role="region" aria-label={t("Benchmark summary")}>
      ...
      <table className="summary-table">
        <tbody>
          <tr><th>{t("Normalized exact / leaf / row")}</th><td colSpan={3}>{pct(summary.exactPassRateNormalized)} / {pct(summary.avgLeafAccuracyNormalized)} / {pct(summary.rowAccuracyNormalized)}</td></tr>
          <tr><th>{t("Completed runs")}</th><td colSpan={3}>{summary.completedRuns}</td></tr>
          <tr><th>{t("Cost per schema-valid / projected per 1,000")}</th><td colSpan={3}>{usd(summary.cost.costPerSchemaValid)} / {usd(summary.cost.projectedPer1000)}</td></tr>
          <tr>
