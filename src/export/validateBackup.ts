...
const snapshot = object({ inputImages: { type: "array", minItems: 1, items: object({ mimeType: enumeration("image/png", "image/jpeg", "image/webp"), dataUrl: str }) }, document, inputBase64: str, profile, golden, provider, effectivePrompt: str, effectiveSchema: {}, pricing: { anyOf: [pricing, { type: "null" }] }, settings: {} }, ["document", "inputBase64", "profile", "provider", "effectivePrompt", "effectiveSchema", "pricing", "settings"]);
...
const attempt = optional({ number: positive, startedAt: date, finishedAt: date, latencyMs: num, raw: str, envelope: str, usage, costUsd: num, costSource: enumeration("provider_reported", "usage_snapshot", "flat", "unknown"), error, parseError: str, providerAttempts: { type: "array", items: {} } }, ["number", "startedAt"]);
...
export async function validateBackupEntities(entities: BackupEntities): Promise<void> {
  ...
  for (const record of entities.benchmarkRuns) {
    ...
    if (record.attempts) {
      const childCalls = record.attempts.reduce((sum, attempt) => sum + (attempt.providerAttempts?.length ?? 1), 0);
      if (childCalls !== record.providerCalls || record.attempts.some((a, i) => a.number !== i + 1)) fail();
    }
