...
## 2026-08-17 — Phase 8 Demo-First Home

- Home's first-time state is now a **Try Demo** card (`DemoBenchmarkCard.tsx`), not an onboarding
  checklist: a bundled synthetic purchase order (`demo/popular-po/` — PDF + prompt + JSON schema +
  expected result, generated by `scripts/generate-demo-pdf.mjs`) is always ready, so a new visitor
  only chooses a provider/model, pastes an API key, picks 1/3/5 runs, and clicks Run Benchmark —
  no PDF upload, no Library/Settings visit, no understanding of Documents/Templates/Expected
  Results required first.
- The demo fixture is idempotently seeded into local storage on first run
  (`src/demo/seedDemoFixture.ts`, fixed ids) and executed through the exact same `BenchmarkRunner`
  every other benchmark uses — not a separate demo engine — so results are real, versioned run
  history, inspectable afterward in Runs & Results like any other benchmark.
- Results render inline: schema-valid/exact-match counts, field/row accuracy, stability, unique
  variants, average latency, and a failures list (run number, field path, expected vs. actual).
- Custom-document benchmarking is demoted to a subordinate "Upload my document" link into the
  existing guided wizard — unchanged, still fully available, just no longer the first thing a new
  visitor has to do.
- Found and fixed a real pre-existing bug while wiring this: `canonical_images` input mode
  (required by OpenAI and OpenAI-compatible, since neither accepts native PDF) had no
  `pdfLoader`/`pageRenderer` wired anywhere outside tests — every production run with those two
  provider kinds would throw "PDF loader unavailable." Added `browserExecuteDeps()`
  (`src/documents/runtimeDeps.ts`) and wired it into the wizard, the repeated-benchmark section,
  and the new demo card.
- Added `tests/e2e/demo.spec.ts`: runs the Home demo through the real pipeline end to end (real
