...
The first-run browser profile now also receives a keyless Gateway Demo provider configuration, which the wizard auto-selects when no other provider is configured. The first Test or Run automatically obtains a short-lived session when needed; the Settings connection action remains available for an immediate connection or manual reconnect, and an intentional removal is preserved.

Gateway output-limit follow-up: the real Nexabyte Pages-origin wizard reaches the image route but terminates at 800 output tokens (`response.incomplete`, `max_output_tokens`). The adapter retains partial output/usage and rejects automatic replay with a specific localized limit message. Completing the unchanged full schema requires a higher gateway output allowance; TASK-068 remains blocked.

Request contract: Gateway Demo never sends output-token-limit parameters, including connection probes and all map/reducer calls. The unused frontend output-limit constant was removed; regression tests enforce omission. The observed 800-token truncation is a server response, not a frontend setting.
